...
public class US014_TC04 extends TestBaseRapor {
    @Test
    public void test() throws InterruptedException, IOException {
        extentTest=extentReports.createTest("US014_TC04", "Exclude sale items / bazi satis ürünleri disinda tut");
        ...
        // "Exclude sale items" seçeneğine tıklanır
        actions.sendKeys(Keys.PAGE_DOWN).sendKeys(Keys.ARROW_DOWN).sendKeys(Keys.ARROW_DOWN).perform();

        Assert.assertTrue(page.cboxExcludeSaleITems.isEnabled());
        extentTest.info("Exclude sale items seçeneğinin erisilebilir oldugu dogrulandı");

        page.cboxExcludeSaleITems.click();
        extentTest.pass("Exclude sale items seçeneğine tıklandı");
        Thread.sleep(1000);

        ReusableMethods.getScreenshot("US014_TC04");
    }
}
