...
public class US015_TC01 extends TestBaseRapor {
    ...
    @Test
    public void Test_US015_TC01() throws Exception {
        extentTest=extentReports.createTest("US015_TC01", "Limitlemeler - Usage limit per coupon / kupon basina kullanım limiti eklenebilmeli ");
        ...
        wait.until(ExceptedCondition.visibilityOf(page.usageLimitWE));

        //Acilan sayfada "Usage limit per coupon" kutusunun enabled oldugu dogrulanmalı
        Assert.assertTrue(page.usageLimitWE.isEnabled());
        extentTest.info("Limit sayfasında 'Usage limit per coupon' kutusunun Enabled oldugu dogrulandı");
        ReusableMethods.getScreenshot("US015_TC01");
        Driver.closeDriver();
    }
}
