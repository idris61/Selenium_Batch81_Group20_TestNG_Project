...
public class US015_TC02 extends TestBaseRapor {
    ...
    @Test
    public void Test_US015_TC02() throws Exception {
        extentTest=extentReports.createTest("US015_TC02", "Limitlemeler - Limit usage to X items");
        ...
        wait.until(ExceptedCondition.visibilityOf(page.limitUsageToXItemsWE));

        //Acilan sayfada "Limit usage to X items" kutusunun enabled oldugu dogrulanmalı
        Assert.assertTrue(page.limitUsageToXItemsWE.isEnabled());
        extentTest.info("Limit sayfasında 'Limit usage to X items' kutusunun Enabled oldugu dogrulandı");
        ReusableMethods.getScreenshot("US015_TC02");
        Driver.closeDriver();
    }
}
